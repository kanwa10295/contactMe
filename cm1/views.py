...
def add_contact(request):
    if request.method == 'POST':
        contact_list = []
        contact_name = request.POST.getlist('name')
        contact_phone = request.POST.getlist('phone')

        zipped = zip(contact_name,contact_phone)

        for contact_name,contact_phone in zipped:
            contact = Contact.objects.create(user=contact_name,
                                             contact =contact_phone)
            contact_list.append(str(contact.id))

        redirect = HttpResponseRedirect(reverse('contact:view_contact'))
        redirect['Location'] += '&'.join(['students={}'.format(x) for x in contact_list])
        return redirect
    else:
        return render("no contact added")

    def view_contact(request):
        contact = request.GET.getlist('contact')
        if contact:
            contact = [int(x) for x in contact]
            instances = Contact.objects.filter(id__in=contact)